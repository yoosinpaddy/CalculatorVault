...
public class CalculatorPresenter implements CalculatorContract.Presenter{
    ...
    public void onExpressionSignChange() {
        ...
        if (mCurrentStringExpression.startsWith("-")){
            isNumberPositive = false;
        }else {
            isNumberPositive = true;
        }

        mCurrentStringExpression = isNumberPositive ? "-" + mCurrentStringExpression : mCurrentStringExpression.substring(1);
    ...
    public void onCalculateResult() {
        if (mCurrentStringExpression.isEmpty() || mCurrentStringExpression.contains(INFINITY)) {
            ...
            if (mCurrentStringExpression.matches(".*[0-9].*")) {
                try {
                    BigDecimal bigDecimalResult = expression.eval();
                    doubleResult = bigDecimalResult.doubleValue();
            ...
            if (isValueInteger(doubleResult) && !isScientificNotation(Double.toString(doubleResult))) {
                long roundedValue = Math.round(doubleResult);
