package com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview;
...
import com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.utils.ResourceUtils;
...
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_HEIGHT_BIAS;
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_SQUARE;
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.AspectRatio.ASPECT_RATIO_WIDTH_BIAS;
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.PatternViewMode.AUTO_DRAW;
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.PatternViewMode.CORRECT;
import static com.calculator.vault.gallery.locker.hide.data.appLock.patternlockview.PatternLockView.PatternViewMode.WRONG;
...
public class PatternLockView extends View {
    ...
    private Rect[][] circles = new Rect[3][3];
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            drawable = typedArray.getDrawable(R.styleable.PatternLockView_dotDrawable);
    ...
    private void setupCircles() {
        ...
        int radius = mDotNormalSize;
        int diameter = (mDotNormalSize * 2);

        circles[0][0] = new Rect(leftX, topY, leftX + diameter, topY + diameter);
        circles[0][1] = new Rect(centerX - radius, topY, centerX + radius, topY + diameter);
        circles[0][2] = new Rect(rightX - diameter, topY, rightX, topY + diameter);
        circles[1][0] = new Rect(leftX, centerY - radius, leftX + diameter, centerY + radius);
        circles[1][1] = new Rect(centerX - radius, centerY - radius, centerX + radius, centerY + radius);
        circles[1][2] = new Rect(rightX - diameter, centerY - radius, rightX, centerY + radius);
        circles[2][0] = new Rect(leftX, bottomY - diameter, leftX + diameter, bottomY);
        circles[2][1] = new Rect(centerX - radius, bottomY - diameter, centerX + radius, bottomY);
        circles[2][2] = new Rect(rightX - diameter, bottomY - diameter, rightX, bottomY);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawPath) {
            ...
            for (int i = 0; i < patternSize; i++) {
                ...
                float centerY = 0;
                if (drawable != null){
                    centerX = circles[dot.mRow][dot.mColumn].centerX();
                    centerY = circles[dot.mRow][dot.mColumn].centerY();
                }else {
                    centerX = getCenterXForColumn(dot.mColumn);
                    centerY = getCenterYForRow(dot.mRow);
                }
                if (i != 0) {
        ...
        for (int i = 0; i < sDotCount; i++) {
            ...
            for (int j = 0; j < sDotCount; j++) {
                ...
                if (drawable == null) {
                    ...
                    drawable.setBounds(circles[i][j].left, circles[i][j].top, circles[i][j].right, circles[i][j].bottom);
    ...
    protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
        ...
        leftX = (int) (getPaddingLeft() + (mViewWidth/2.5));
        topY = (int) (getPaddingTop() + (mViewHeight/2.5));
        rightX = (int) (width - getPaddingRight() - (mViewWidth/2.5));
        bottomY = (int) (height - getPaddingBottom() - (mViewHeight/2.5));
